...
import { X, Check, ChevronLeft, ChevronRight, Plus, Trash2, Repeat, Loader2, TimerReset } from 'lucide-react'
...
export default function SesionActiva({
  ...
  useEffect(() => {
    ...
    async function cargarPrevias() {
      ...
      const { data } = await supabase
        ...
        .limit(200)
      if (!data) return
      const ultimaPorEjercicio = new Map<string, { peso: number; reps: number }>()
      for (const r of data as { ejercicio_id: string; peso: number; reps: number }[]) {
      ...
      setEjercicios((prev) =>
        prev.map((e) => {
          ...
          return u ? { ...e, previa: `${u.peso} kg x ${u.reps}` } : e
        })
      )
    }
    cargarPrevias()
  ...
  const totalSets = ejercicios.reduce((s, e) => s + e.sets.length, 0)
  const setsCompletados = ejercicios.reduce((s, e) => s + e.sets.filter((x) => x.completado).length, 0)
  ...
  async function terminarSesion() {
    ...
    if (filas.length === 0) {
      toast.error('No marcaste ninguna serie como hecha')
      setTerminando(false)
      return
    }
    ...
    await otorgarXP(supabase, perfil, {
      ...
      p_descripcion: `${rutina.nombre} · ${filas.length} series`,
    })

    toast.success(`Entrenamiento guardado: ${filas.length} series`)
    router.refresh()
    setTerminando(false)
    onCerrar()
  }
  ...
  return (
    <div className="fixed inset-0 bg-background z-[70] flex flex-col">
      ...
      <div className="flex items-center gap-2 px-4 py-3 overflow-x-auto border-b border-border">
        {ejercicios.map((e, i) => {
          const hecho = e.sets.length > 0 && e.sets.every((s) => s.completado)
          return (
            <button
        ...
        <div>
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <span className="pill capitalize mt-1 inline-block">{actual.ejercicio.grupo_muscular}</span>
            </div>
